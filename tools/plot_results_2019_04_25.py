...
def _plot_gan(ax, base_data, new_data, avg=False):
    _plot_gan_axis(ax, base_data, ['r', 'b', 'g'], prefix='Before', avg=avg)
    _plot_gan_axis(ax, new_data, ['m', 'c', 'y'], prefix='After', avg=avg)
    ax.grid()
    ax.set_yscale('log')
...
def _plot_feats(base_data, new_data):
    ...
    _plot_envelopped(
        ...
        label='Before: Training - Input vs Reconstructed', color='b')
    _plot_envelopped(
        ax, new_data.train['STEP'], new_data.train['F_RECON'],
        label='After: Training - Input vs Reconstructed', color='r')
    ax.plot(
        base_data.test['STEP'], base_data.test['F_RECON'],
        label='Before: Test - Input vs Reconstructed', color='c')
    ax.plot(
        new_data.test['STEP'], new_data.test['F_RECON'],
        label='After: Test - Input vs Reconstructed', color='m')
    ax.grid()
    ax.legend()
    ax.set(
        xlabel='Steps',
        ylabel='Feature Matching Error',
        title='Feature Matching Error')
    return fig
